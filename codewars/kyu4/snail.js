snail = function(array) {
    let snailArray = []
    while(array.length) {
      snailArray.push(...array.shift()); //start by taking the numbers from the top
      for (let i = 0; i < array.length; i++){ // next we grab the number from the right side (yop yo bottom)
        snailArray.push(array[i].pop())
      }
      snailArray.push(...(array.pop() || []).reverse()) // then we grab the numbers across the bottom and reverse them
      for (let i = array.length -1; i >= 0; i--) { //lastly we grab the numbers from the left side
